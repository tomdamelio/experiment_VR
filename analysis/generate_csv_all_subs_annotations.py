...
def modify_annotations(row):
    annotations = ast.literal_eval(row['continuous_annotation'])
    if row['order_emojis_slider'] == 'inverse':
        annotations = [[-value, time] for value, time in annotations]
    return str(annotations)  # Convertir de nuevo a cadena para mantener la consistencia en el DataFrame

# Asegurar que 'continuous_annotation' está en formato de listas y aplicar la transformación
data_all['continuous_annotation'] = data_all.apply(modify_annotations, axis=1)
