...
def get_video_files_from_csv(csv_path):
    ...
    # Convert relative paths to absolute paths, making sure we use the correct base directory
    # Assuming the paths in the CSV are relative to the directory where the CSV file is located
    csv_base_dir = os.path.dirname(csv_path)
    video_files = [os.path.abspath(os.path.join(csv_base_dir, path)) for path in video_files]
